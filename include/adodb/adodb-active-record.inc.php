...
class ADODB_Active_Record {
    ...
    function Load($where=null,$bindarr=false, $lock = false)
    {
        ...
        $qry = sprintf(
            "SELECT * FROM %s",
            strtolower($this->nameQuoter($db, $this->_table))
        );
    ...
    function Insert()
    {
        ...
        foreach ($table->flds as $name => $fld) {
            $prop = lcfirst($fld->name);
            $val  = property_exists($this, $prop) ? $this->$prop : null;
            echo "Debug: Field: $name, Property: $prop, Value: " . var_export($val, true) . "<br>";

            if ($fld->primary_key && $fld->auto_increment) {
                echo "Debug: Skipping auto-increment field: $name<br>";
                continue;
            }
        ...
        $tableName = strtolower($this->nameQuoter($db, $this->_table));
        ...
        if ($ok) {
            ...
            foreach ($table->keys as $k) {
                ...
                if ($fld->auto_increment && isset($this->$prop) && is_null($this->$prop)) {
                    $this->$prop = $this->LastInsertID($db, $k);
                    echo "Debug: Set auto-increment ID: " . $this->$prop . "<br>";
                }
            }
        }
    ...
    function Delete()
    {
        $db = $this->DB();
        if (!$db) {
            echo "DEBUG: No database connection<br>";
            //  return false;
        }
        ...
        echo "DEBUG: Table: " . $this->_table . "<br>";
        echo "DEBUG: Primary keys: ";
        print_r($table->keys);
        echo "<br>";

        // DEBUG: Check current object values
        echo "DEBUG: Current object values:<br>";
        foreach ($table->keys as $key) {
            echo " - $key: " . $this->$key . "<br>";
        }

        $where = $this->GenWhere($db, $table);
        echo "DEBUG: Generated WHERE clause: " . $where . "<br>";

        if (empty($where)) {
            echo "DEBUG: WHERE clause is EMPTY - cannot delete without primary key values<br>";
            //return false;
        }
        ...
        $ok = $db->Execute($sql);
        echo "DEBUG: Execute result: " . ($ok ? "SUCCESS" : "FAILED") . "<br>";

        if (!$ok) {
            echo "DEBUG: Error: " . $db->ErrorMsg() . "<br>";
        }
        die();
